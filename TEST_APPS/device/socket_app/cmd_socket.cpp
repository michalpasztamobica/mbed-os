...
#define MAN_SOCKET          "\r\nSOCKET API\r\n"\
                            ...
                            " new <type>\r\n" \
                            "   type: UDPSocket|TCPSocket|TCPServer|TLSSocket [--cert_url <url>|--cert_file <file>|--cert_default]\r\n"\
                            "   return socket id\r\n"\
                            " <id> delete\r\n"\
...
static int get_cert_from_url(const char *url, char **cert)
{
    NetworkInterface *iface = get_interface();
    if (!iface) {
        cmd_printf("No interface defined\r\n");
        return CMDLINE_RETCODE_FAIL;
    }

    HTTPClient http(iface);
    HTTPResult ret = http.get(url, *cert, CERT_BUFFER_SIZE);
    if (ret == HTTP_OK) {
        tr_debug("HTTP_GET: Read %d chars: '%s' ... [OK]\r\n", strlen(*cert), *cert);
...
static int tls_set_cert(int argc, char *argv[], SInfo *info)
{
    ...
    char *ptr_cert = NULL;
    char *src = NULL;
    if (cmd_parameter_val(argc, argv, "--cert_url", &src)) {
        tr_debug("Root ca certificate read from url: %s", src);
        ptr_cert = read_cert;
        if (get_cert_from_url(src, &ptr_cert) == CMDLINE_RETCODE_FAIL) {
            cmd_printf("Cannot read from url: %s\r\n", src);
            return CMDLINE_RETCODE_INVALID_PARAMETERS;
        }
    } else if (cmd_parameter_val(argc, argv, "--cert_file", &src)) {
